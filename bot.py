...
if __name__ == "__main__":
    main()    c.execute("SELECT verb FROM progress WHERE day = ?", (day,))
    rows = c.fetchall()
    conn.close()
    return [r[0] for r in rows]
...
def main():
    ...
    # Schedule: 9:00 and 20:00 UTC (Portugal = UTC+1 summer, UTC+0 winter)
    # Using UTC 8:00 and 19:00 to approximate Portugal morning/evening
    job_queue = app.job_queue
    job_queue.run_daily(scheduled_morning, time=time(8, 0))
    job_queue.run_daily(scheduled_evening, time=time(19, 0))
